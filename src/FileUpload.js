  ...
  csvToJson = (csv) => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (Object.keys(obj).length && lines[i].trim()) {
        const parsedObj = {
          // why was this such a pain, kept converting my stuff to eastern time, had to do this to fix it
          Date: new Date(2024, new Date(obj.Date).getUTCMonth(), 1),
          GPT4: Number(obj["GPT-4"]),
          Gemini: Number(obj.Gemini),
          PaLM2: Number(obj["PaLM-2"]),
          Claude: Number(obj.Claude),
          LLaMA3_1: Number(obj["LLaMA-3.1"])
        };
